...
SELECT NOMBRE AS Nombre_Completo, SEXO AS Género, EDAD AS Años, DIRECCION1 AS Domicilio FROM tb_cliente;#aca se estan utilizando el alias, es cambiar el nombre con AS pero no es cambiarlo de forma en la tabla solo en el SQL 
SELECT NOMBRE, SEXO, EDAD, DIRECCION1 FROM tb_cliente LIMIT 6;#que me muestre los datos que yo quiero pero no todos con limit podemos elegir la cantidad
select nombre, matricula from tb_vendedor limit 2;

SELECT * FROM tb_cliente;
SELECT * FROM tb_producto WHERE SABOR = 'Maracuyá'; #un select con filtro, muestra todos los datos que tenga Maracuya
UPDATE tb_producto SET SABOR = 'Cítrico' WHERE SABOR = 'Limón';

select * from tb_vendedor;
select * from tb_vendedor where nombre = " Claudia Morais";

SELECT * FROM tb_cliente;

SELECT * FROM tb_cliente WHERE EDAD > 27;# buscar un dato por edad pero que se mayor o tambien se puede mayor o igual <=


SELECT * FROM tb_cliente WHERE EDAD <> 26;# que sean mayor y menor pero no mostrara los de 26
